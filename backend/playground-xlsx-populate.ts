...
async function playgroundXlsxPopulate() {
    ...
    try {
        ...
        console.log(`  Rows: ${sourceSheet.usedRange()?.endCell().rowNumber() || 'unknown'}`);
        console.log(`  Cols: ${sourceSheet.usedRange()?.endCell().columnNumber() || 'unknown'}`);
